...
As soon as the accumulated diff passes roughly 250 lines, announce the review rather than launch it:
say the threshold is crossed, there and not at the end of the task, and let the author pick the
moment — fired mid-thought, a review reports against code that was about to change anyway. Delegate
it to a **read-only** subagent on a reasoning-heavy model — Opus at minimum, or Fable, never a
smaller tier — and say the read-only constraint in its prompt: no `git add / checkout / restore /
stash / reset / clean / commit / push / merge`, no file write, no outward-facing action. Verify its
findings before acting on them; it is sometimes wrong.
